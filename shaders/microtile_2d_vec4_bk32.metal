...
// BK=32 sweep variant of the v5 Vec4 kernel.
// Each thread loads two float4 chunks per A tile and two per B tile.
kernel void sgemm_microtile_2d_vec4_bk32(const device float *A [[buffer(0)]],
                                         ...
                                         uint2 tid    [[thread_position_in_threadgroup]])
{
    ...
    for (uint k0 = 0; k0 < dims.k; k0 += BK) {
        ...
        for (uint load_i = 0; load_i < 2; ++load_i) {
            uint idx = (linear_tid * 2u + load_i) * 4u;
            uint a_row = idx / BK;
            uint a_col = idx % BK;
